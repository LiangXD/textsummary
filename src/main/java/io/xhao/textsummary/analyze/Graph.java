...
class Graph {
	...
	/*
	 * private void genContriTokens(Cluster[] clusters) { for(Cluster c :
	 * clusters){ for(WrapToken t : c.getTokens()){
	...
	/*
	 * private WrapToken[] findLinkTokens(WrapToken[] remain, WrapToken[] freq,
	 * Cluster[] clusters){ if(isConnectGraph(clusters)) { log.debug(
	 * "this is a connectted graph, maybe no links exist."); return new
	 * WrapToken[0]; } Map<WrapToken, Double> tokens = new HashMap<WrapToken,
	 * Double>(); List<Double> scores = new ArrayList<Double>(); for(WrapToken w
	 * : remain) { double score = 1; for(Cluster c : clusters){ score = score *
	 * (1 - (double) Utility.calcContribution(w, c) / (double)
	 * c.getContributionToArticle()); } tokens.put(w, 1 - score);
	 * scores.add(score); } for(WrapToken w : freq) { double score = 1;
	 * for(Cluster c : clusters){ score = score * (1 - (double)
	 * Utility.calcContribution(w, c) / (double) c.getContributionToArticle());
	 * } tokens.put(w, 1 - score); scores.add(score); } int n = (int)
	 * (tokens.size() * SummaryConstant.LINK_RATIO);
	 * Utility.popup(scores.toArray(new Double[0]), n); ArrayList<WrapToken>
	 * cand = new ArrayList<WrapToken>(); for(Entry<WrapToken, Double> entry :
	 * tokens.entrySet()) { if(entry.getValue() >= level){
	 * cand.add(entry.getKey()); } } return cand.toArray(new WrapToken[0]); }
	 */
